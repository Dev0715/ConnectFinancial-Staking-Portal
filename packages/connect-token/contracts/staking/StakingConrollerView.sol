...
contract StakingControllerTemplateView {
    using SafeMathUpgradeable for *;
    StakingControllerLib.Isolate isolate;

    // function getWeightAndRelease() public view returns (uint256[2] memory) {
    //     return [isolate.totalWeight, isolate.inflateBy];
    // }

    function getWeightAndRelease() public view returns (uint256) {
        return isolate.currentCycle;
    }
        ...
        )
    {
        ...
        if (caller != address(0x0)) {
            ...
            StakingControllerLib.UserWeightChanges
                storage _weightChange = isolate.weightChanges[caller];
            returnstats.totalCyclesSeen = _weightChange.totalCyclesSeen;

            locals.tierIndex = Math.max(
                dailyUser.commitment,
                dailyUser.currentTier
            );
            ...
            }
            {
                ...
                (, returnstats.bonuses) = CalculateRewardsImplLib
                    ._computeRewards(isolate, caller);
            }
        }
        _cycle = isolate.cycles[isolate.currentCycle];
        ...
        returnstats.commitmentViolationPenalty = isolate
            .commitmentViolationPenalty;
